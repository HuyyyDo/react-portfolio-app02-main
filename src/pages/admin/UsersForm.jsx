...
export default function UserForm(){
  ...
  React.useEffect(()=>{
    if(!isEdit) return
    let alive = true
    api.get(RES.users, id).then(d=>{{ if(alive) setForm(d) }}).catch(e=>setError(e.message)).finally(()=>setLoading(false))
    return ()=>{{ alive=false }}
  },[id])
  ...
  async function onSubmit(e){
    ...
    setSaving(true)
    try{
      if(isEdit) await api.update(RES.users, id, form)
      else await api.create(RES.users, form)
  ...
  return (
    <form className="card" onSubmit={onSubmit}>
      <h3 style={{marginTop:0}}>'User' – {isEdit? 'Edit':'Add'}</h3>
